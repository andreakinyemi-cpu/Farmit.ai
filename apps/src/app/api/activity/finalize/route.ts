...
import { addActivityEventDemo, addExportDemo, createActivityDemo, createFarmDemo, listFarms } from "@/lib/demoStore";
...
function resolveFarmIdDemo(userId: string, farmId?: string) {
  if (farmId) return farmId;
  const farms = listFarms(userId);
  if (farms[0]?.id) return farms[0].id;
  return createFarmDemo(userId, "Default Farm").id;
}
...
export async function POST(req: Request) {
  try {
    ...
    let farmId: string;
    let field: { acreage?: number } | null = null;
    let catalog: {
    ...
    try {
      farmId = await resolveFarmId(userId, body.farmId);
      field = fieldId ? await getFieldById(fieldId) : null;
      const productName = String((body.draft.data as Record<string, unknown>).product_name || body.draft.entities?.product_mention || "");
      catalog = productName ? (await findCatalogCandidates(productName, 1))[0] : null;
    } catch {
      farmId = resolveFarmIdDemo(userId, body.farmId);
    }
    ...
    if ((missing.length > 0 || hard_errors.length > 0) && body.status === "final") {
      return fail("VALIDATION_FAILED", "Record has missing or invalid fields", { missing_fields: missing, hard_errors, flags }, 422);
    }
    ...
    try {
      ...
      return ok({ record_id: created.id, farm_id: farmId, status: created.status, missing_fields: missing, flags, export: exportPath });
    } catch {
      const created = createActivityDemo({
        farm_id: farmId,
        field_id: fieldId,
        type: body.draft.type,
        occurred_at: body.draft.occurred_at,
        created_by: userId,
        status: body.status,
        gps_lat: body.gps?.lat ?? body.draft.gps?.lat,
        gps_lon: body.gps?.lon ?? body.draft.gps?.lon,
        transcript: body.transcript,
        data: body.draft,
        finalized_at: body.status === "final" ? new Date().toISOString() : undefined,
      });
      addActivityEventDemo(created.id, "validation", { missing_fields: missing, flags, hard_errors });
      if (body.override_reason) addActivityEventDemo(created.id, "override_reason", { reason: body.override_reason });

      let exportPath: string | null = null;
      if (body.status === "final") {
        exportPath = `/exports/demo-${created.id}.pdf`;
        addExportDemo(farmId, "activity_pdf", exportPath, created.id);
      }

      return ok({ record_id: created.id, farm_id: farmId, status: created.status, missing_fields: missing, flags, export: exportPath, degraded: true });
    }
  } catch (err: unknown) {
