...
function fallbackParse(transcript: string, timestamp?: string) {
  return {
    type: "other",
    ...
    missing_fields: [],
    confidence: {},
    normalization_notes: ["fallback_parse_used"],
  };
}
...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      const activity = fallbackParse(body.transcript, body.timestamp);
      return ok({
        activity,
        missing_fields: activity.missing_fields,
        confidence: activity.confidence,
